(function (factory) {
    ...
    $.fn.carousel = function(options){
        var _defaults = {
                ...
                animationCurve: 'ease-in',     /* ease | ease-in | ease-out | ease-in-out | linear | bezier */
        ...
        var _carouselDiv = $( "<div class='carousel' />" )
        ...
        (function setup(){
            ...
            function createcarousel(){
                ...
                /* create carousel slide stack */
                if(_settings.slides.length){                    
                    for(var i = 0;  i < _settings.slides.length; i++){
                        var slide = $( "<div class='carousel' data-index="+i+" />" )
                ...
                _carouselDiv.find('.'+_settings.slideClass).css({position: 'absolute', left: 0, top:0, width:'100%', height:'100%', backfaceVisibility: 'hidden'})
            ...
            function _createNavigation(){
                ...
                _carouselDiv.after(_navigation);
                ...
                _container.on('click', '.nav__item', function(e){
                    _targetSlideIndex =  $(this).index();
                    _animations.run(_settings.animation, _targetSlideIndex);
                    e.preventDefault();
                });
        ...
        function _getPreviousSlide(){
            return _carouselDiv.find('.'+_settings.slideClass).eq((_currentSlideIndex-1)%_noOfSlides);
        }
        function _getCurrentSlide(){
            return _carouselDiv.find('.'+_settings.slideClass).eq(_currentSlideIndex);
        }
        function _getNextSlide(){
            return _carouselDiv.find('.'+_settings.slideClass).eq((_currentSlideIndex+1)%_noOfSlides);
        }
        function _getSlideByIndex(idx){
            return _carouselDiv.find('.'+_settings.slideClass+'[data-index='+idx+']');
        }
        ...
        function _slide(targetSlideIndex){
            ...
            _carouselDiv.css({ transform: 'translateZ('+-_translateZ+'px) rotateY('+-_rotationAngle+'deg)' });
            _slideCarouseld();
        }
        ...
        function _scroll(targetSlideIndex){
            _container.css({ perspective: '', overflow: 'hidden' });
            _rotationAngle=_baseAngle * targetSlideIndex;
            _carouselDiv.css({ transform: 'translateZ('+-_translateZ+'px) rotateX('+-_rotationAngle+'deg)' });
            _slideCarouseld();
        }
        ...
        function _fade(targetSlideIndex){
            _carouselDiv.css({ transition: 'opacity '+ _settings.animationDuration+'ms '+_settings.animationCurve, opacity: 0});
            /* set active nav icon */
            ...
            var _tt = setTimeout(function(){
                _rotationAngle = _baseAngle * targetSlideIndex;
                _carouselDiv.css({ transform: 'translateZ('+-_translateZ+'px) rotateY('+-_rotationAngle+'deg)', opacity: 1 });
        ...
        this.showSlide = function(index){
            _carouselDiv.find('.nav').eq((index-1)%_noOfSlides).click();
        }
